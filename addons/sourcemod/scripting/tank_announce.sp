...
int  g_iTimerDelay = 0;                     // Workaround for announce multiple tanks
...
public void L4D_OnSpawnTank_Post(int iClient, const float vPos[3], const float vAng[3])
{
    ...
    DataPack hPack;
    CreateDataTimer(fDelay, Timer_Announce, hPack, TIMER_FLAG_NO_MAPCHANGE);
    hPack.WriteCell(iUserId);
    hPack.WriteCell(g_iTankIdx);
...
Action Timer_Announce(Handle hTimer, DataPack hPack)
{
    if (g_iTimerDelay > 0) {
        g_iTimerDelay--;
    }
